...
<head>
  ...
  <title>ChampiLoco(Eliminar empleado)</title>
...
<!-- Sección de empleados -->
  ...
  if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['id'])) {
      $id = $_POST["id"];
      $stmt = $enlace->prepare("DELETE FROM usuarios WHERE id=? AND tipoUsuario = 2");
  ...
  echo "<h2>Eliminar Empleado</h2>";
  while ($row = $result->fetch_assoc()) {
      echo "<form method='post' style='display:inline;'>
          <input type='hidden' name='id' value='{$row['id']}'>
          <strong>{$row['nombre']}</strong> ({$row['usuarioN']})
